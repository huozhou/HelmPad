 ...
 *     says the user finished the 5-step wizard. No HID traffic is started yet.
...
private fun OperatorRoute(app: VibePadApplication) {
    ...
    // Load profiles off the main thread. Reuse the cached result across recompositions.
    // Diagnostics from the loader are folded into the failure message for the
    // ErrorScaffold; we no longer surface them separately in Settings (the only
    // bundled profile is validated by a unit test before each build, so that
    // panel was always empty in practice).
    ...
    LaunchedEffect(Unit) {
        profileLoadResult = try {
            ...
            val chosen = outcome.profiles.firstOrNull { it.id == BUNDLED_PROFILE_ID }
                ?: outcome.profiles.firstOrNull()
            if (chosen == null) {
...
private fun OperatorContent(
    ...
    val vm: OperatorViewModel = viewModel(factory = OperatorViewModel.factory(app, profile))
...
private const val BUNDLED_PROFILE_ID = "profile.claude-code"
...
private sealed interface ProfileLoadResult {
    ...
    data class Ok(val profile: Profile) : ProfileLoadResult
}
